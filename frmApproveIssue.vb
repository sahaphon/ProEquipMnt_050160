...
Private Sub PreGroupType()
 ...
 Dim i As Byte
     ...
     strGpTopic(0) = "รหัสเอกสาร"
     strGpTopic(1) = "รายละเอียดอุปกณ์"           'ชื่ออุปกรณ์
     strGpTopic(2) = "แผนกที่แจ้งปัญหา"
     strGpTopic(3) = "รายละเอียดกลุ่ม"            'DescThai
     strGpTopic(4) = "รุ่นอุปกรณ์"                'รุุ่นอุปกรณ์
                              ...
                              Select Case frmMainPro.lblLogin.Text
                                     ...
                                     Case Is = "PRADIST"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 ...
                                                                 & " AND dep_notify = '" & "122000 แผนกตัดชิ้นส่วน" & "'" _
                                     ...
                                     Case Is = "ITHISAK"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 & " WHERE " & strFieldFilter _
                                                                 & " AND dep_notify = '" & "125000 แผนกฉีด EVA INJECTION" & "'" _
                                                                 & " AND person1_sta = 'True' " _
                                                                 & " ORDER BY req_id"
                                     ...
                                     Case Is = "TODSAPORN"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 & " WHERE " & strFieldFilter _
                                                                 & " AND dep_notify = '" & "126000 แผนกฉีด PU" & "'" _
                                                                 & " AND person1_sta = 'True' " _
                                                                 & " ORDER BY req_id"
                                     ...
                                     Case Is = "SATHID"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 & " WHERE " & strFieldFilter _
                                                                 & " AND dep_notify = '" & "123000 แผนกเย็บ" & "'" _
                                                                 & " AND person1_sta = 'True' " _
                                                                 & " ORDER BY req_id"
                                     ...
                                     Case Is = "TECHIN"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 & " WHERE " & strFieldFilter _
                                                                 & " AND dep_notify = '" & "121000 แผนกผลิตโฟม" & "'" _
                                                                 & " AND person1_sta = 'True' " _
                                                                 & " ORDER BY req_id"
                                     Case Is = "PEERA"
                                         ...
                                         strSqlCmdSelc = "SELECT  * FROM v_notifyissue (NOLOCK)" _
                                                                 & " WHERE " & strFieldFilter _
                                                                 & " AND dep_notify = '" & "124000 แผนกฉีด PVC" & "'" _
                                                                 & " AND person1_sta = 'True' " _
                                                                 & " ORDER BY req_id"
                                     ...
                                     '-------------------------------------------สถานะส่งมอบฝ่ายผลิต---------------------------
                                                 ...
                                                 If frmMainPro.lblLogin.Text = "BOONTUM" Then
                                                            ...
                                                            End Select
                                                                  ...
                                                                  '----------- ตรวจสอบว่า ผู้อนุมัติรับแจ้ง ยืนยันหรือยัง -----------------------
 ...
 Dim strSearch As String = txtSeek.Text.ToUpper.Trim
   ...
   If strSearch <> "" Then
        ...
        Select Case cmbType.SelectedIndex()
               ...
               Case Is = 2 'แผนกที่แจ้งปัญหา
                    SearchData(2, strSearch)

               Case Is = 3 'รายละเอียดกลุ่ม
                    SearchData(3, strSearch)

               Case Is = 4 'รุ่นอุปกรณ์
